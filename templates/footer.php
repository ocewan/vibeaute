...
<script>
    ...
    const spanUsername = document.querySelector(".user-container span");
    const spanPassword = document.querySelector(".password-container span");
    ...
    if (form) {
        form.addEventListener("submit", (e) => {
            ...
            if (!usernameInput.value.trim()) {
                spanUsername.textContent = "Veuillez entrer votre nom d'utilisateur.";
                hasError = true;
            } else {
            ...
            if (!passwordInput.value.trim()) {
                spanPassword.textContent = "Veuillez entrer votre mot de passe.";
                hasError = true;
            } else {
    ...
    // Affichage du nom du fichier photo sélectionné
    document.addEventListener("DOMContentLoaded", () => {
        const fileInput = document.getElementById("photo-upload");
        const label = document.querySelector('label[for="photo-upload"] span');

        fileInput.addEventListener("change", () => {
            if (fileInput.files.length > 0) {
                label.textContent = fileInput.files[0].name;
            } else {
                label.textContent = "Sélectionner une image";
            }
        });
    });
</script>
